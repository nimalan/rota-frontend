...
const API_URL = 'https://my-rota-api.onrender.com'; // your API base URL
    ...
    <div>
      ...
      >
        ...
        {isRecurring && (
          <label>
            Number of Months:
            <input type="number" min="1" value={recurrenceMonths} onChange={e => setRecurrenceMonths(e.target.value)} />
          </label>
        )}
        <div style={{ marginTop: '1rem' }}>
          <button onClick={handleSave}>Save</button>
          <button onClick={closeModal} style={{ marginLeft: '1rem' }}>Cancel</button>
        </div>
      </Modal>
    </div>
  );
